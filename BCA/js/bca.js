...
function addEntity() {
	...
	var hashPersonal = EthJS.Util.hashPersonalMessage(pubKey, EthJS.Util.bufferToHex(addrb));
	hashPersonal = EthJS.Util.bufferToHex(hashPersonal);
	alert(hashPersonal);

	addrb = EthJS.Util.bufferToHex(addrb);
	pubKey = EthJS.Util.bufferToHex(pubKey);
	// alert(EthJS.Util.hashPersonalMessage(pubKey, EthJS.Util.bufferToHex(addrb)));
	web3.eth.contract(contractAbi).at(contractAddress).addEntity(addrb,pubKey,hashPersonal, function(err,result){
		console.log(result);
	});
...
function getEntity(){
	...
	entityAddress = web3.toHex(entityAddress);
  //web3js = new Web3(web3.currentProvider);
	web3.eth.contract(contractAbi).at(contractAddress).getEntity.call(entityAddress, function(err, result){
	if(!err){
		$("#entityPubKey").append(result);
	}else{
		$("#entityPubKey").append(err);
		console.log(err);
		}
	});
}
...
function getContractAddress(){
	var ownAddr = $("#ownerAddress").val();
	ownAddr = web3.toHex(ownAddr);
	web3.eth.contract(contractAbi).at(contractAddress).getOwnerSignature.call(ownAddr, function(err,result){
		if(!err){
			$("#contractAddress").append(result);
		}else{
			$("#contractAddress").append(err);
			console.log(err);
		}
	});
}
...
function getSignature(){
	...
	contractAddr = web3.toHex(contractAddr);
	var ownerAddr = $("#ownerAddr").val();
	ownerAddr = web3.toHex(ownerAddr);

	web3.eth.contract(contractAbi).at(contractAddress).getSignature.call( contractAddr, ownerAddr, function(err, result){
		var data = result[0];
		if(!err){
			console.log(result[0]);
			$("#certificate").html(result[0]);
			$("#signature").html(result[1]);
			$("#boton-txt").html("<a class='btn btn--xs btn--green-primary type--uppercase' type='button' onclick='createFile();'><span class='btn__text color--white' lang='en'> Download Certificate </span></a><script type='text/javascript' src='js/createFile.js'></script>");
		}else{
			$("#certificate").html(err);
		}
